...
Stencil.js Web Components UI kit for Cloudflare RealtimeKit (RTK), published as `@cloudflare/realtimekit-ui`. Framework wrappers for React and Angular are auto-generated by Stencil output targets on every build; the Vue wrapper is manually maintained (generator intentionally disabled).
...
├── packages/vue-library/     # Manually maintained Vue wrappers (generator disabled)
...
- Vue library is frozen — new components must be manually added to `packages/vue-library/lib/components.ts`.
